...
def get_data(games, get_move, results_file=None):
    """
...
def train(nets, game_batch, passes, evals, valid_batch, path, runs=-1):
    ...
    while count != runs:
        count += 1
        boards, labels, a_boards, results, _ = get_data(game_batch,
                                                     get_move_mcts(nets, evals),
                                                     path + "/output/results_iter_%i.txt" % count)
        b_val, l_val, a_val, r_val, _ = get_data(valid_batch, get_move_mcts(nets, evals))
...
def get_vanilla_data(evals, games):
    ...
    boards, labels, a_boards, results = get_data(games, get_move_mcts(nets, evals), runs_root + "/vanilla/output/results.txt")
    with open_creating_dir_if_needed(vanilla_data_path, "wb") as file:
        pickle.dump([boards, labels, a_boards, results], file)
...
def test_outputs():
    ...
    while True:
        ...
        get_data(10, get_move_mcts(nets, evals), run_path + "last_iter.txt")
...
def train_policy(game_batch, passes, runs):
    ...
    load_path = runs_root + "/run1499671533/checkpoints/" + "predict_iter_1.ckpt"
    ...
    r = 0
    while r != runs:
        to_move_boards, l, _, _, res = get_data(game_batch, get_move_policy, runs_root + "/policy_test/results.txt")
        r += 1
        res = normalise(res)
...
#get_vanilla_data(100, 100)
#train_policy(game_batch=100, passes=10, runs=-1)
